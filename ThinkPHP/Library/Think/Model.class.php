...
class Model {
    ...
    public function add($data='',$options=array(),$replace=false) {
        ...
        if(false !== $result ) {
            $insertId   =   $this->getLastInsID(); // 获取当前信息被插入到数据库之后返回的主键ID
            if($insertId) {
                // 自增主键返回插入ID
    ...
    // 插入数据前的回调方法 只是定义 并没有实现 等着继承它的类去实现
    ...
    public function save($data='',$options=array()) {
        if(empty($data)) {
            // 没有传递数据，获取当前数据对象的值
            // 没有传递参数话 是从$this->data
            // $this->data 是从 create方法来
            if(!empty($this->data)) {
                $data           =   $this->data;
                // 重置数据
                $this->data     =   array();
        ...
        // 更新的前置钩子
        // 前置钩子只要不返回false，后面的代码会继续执行
        if(false === $this->_before_update($data,$options)) {
            return false; // 就不会执行真正的更新操作
        }        
     ...
     */
     public function create($data='',$type='') {
        ...
        // 赋值当前数据对象 当用户提交的post数据存放到model的data属性里面
        // 
        $this->data =   $data;
        // 返回创建的数据以供其他调用
        return $data;
